...
public class LethalLianaFeature extends Feature<NoneFeatureConfiguration> {
    private static final Direction[] DIRECTIONS = Direction.values();

    public LethalLianaFeature(Codec<NoneFeatureConfiguration> pCodec) {
        super(pCodec);
    }
        ...
        for (int i = 0; i <= height; i++) {
            if (level.isEmptyBlock(pos)) {
                if (i == height || !level.isEmptyBlock(pos.below())) {
                    level.setBlock(
                            pos,
                            GarnishedAdditionsBlocksInit.LETHAL_LIANA
                ...
                level.setBlock(pos, GarnishedAdditionsBlocksInit.LETHAL_LIANA_PLANT.get().defaultBlockState(), 2);
            }
